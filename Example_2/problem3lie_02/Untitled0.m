...
y_r=exp(-(0.4.*x)).*sin(x);   %Exact solution y_1 in the training set
yy_r=exp(-(0.4.*x)).*cos(x)-(2./5).*exp(-(0.4.*x)).*sin(x); %Exact solution y_2 in the training set
y_test_r=exp(-(0.4.*q)).*sin(q);   %Exact solution y_1 in the test set
yy_test_r=exp(-(0.4.*q)).*cos(q)-(2./5).*exp(-(0.4.*q)).*sin(q); %Exact solution y_2 in the test set

y_a_r=exp(-(0.4.*B)).*sin(B);   %Exact solution y_1 for random points in the training set
yy_a_r=exp(-(0.4.*B)).*cos(B)-(2./5).*exp(-(0.4.*B)).*sin(B); %Exact solution y_2 for random points in the training set
...
figure(2)
subplot(1,2,1)    %训练集0-2上的 y_1与精确解的误差    以及 y_2与精确解的误差
plot(x,error1,'-');
hold on;
plot(x,error3,'--');
hold on;
...
subplot(1,2,2)     %测试集0-1.5上的 y_1与精确解的误差    以及 y_2与精确解的误差
plot(q,error2,'-');
hold on;
plot(q,error4,'--');
hold on;
