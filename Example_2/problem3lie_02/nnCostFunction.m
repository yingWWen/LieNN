...
%E:Loss function  grad: Updated parameters
w=param(1:n,:);
b=param(n+1:2*n,:);
v=param(2*n+1:3*n,:);
a=param(3*n+1:4*n,:);
...
for i=1:m
    ...
    for j=1:n
        ...
        num=num+u(j).*sigm;
        nom=nom+u(j).*sigm.*(1+(2./5).*x(i)+a(j).*x(i).*(1-sigm));
    end
    tmp=(som-x(i).*num);
    J=J+(0.5/m).*tmp^2;
    tmp_sigma=sigmoid(x(i).*w+b);
    %obtained from the equation
    tmpe=(((2./5).*exp(-(0.4.*x(i))).*cos(x(i)))+nom+x(i).*sum);
    JJ=JJ+(0.5/m).*tmpe^2;
    ...
    grad_a=grad_a + (1/m).*tmp.*(...
        ...
        (1/m).*tmpe.*(2.*x(i).*(u.*tmpe_sigm.*(1-tmpe_sigm))+(2./5).*x(i).*x(i).*(u.*tmpe_sigm.*(1-tmpe_sigm))+...
    ...
    grad_s=grad_s + (1/m).*tmp.*(...
        ...
        (1/m).*tmpe.*((u.*tmpe_sigm.*(1-tmpe_sigm))+(2./5).*x(i).*(u.*tmpe_sigm.*(1-tmpe_sigm))+...
        x(i).*(a.*u.*tmpe_sigm.*(1-tmpe_sigm).*(1-2*tmpe_sigm)));

    %Derivative of E with respect to the bias y_2
    grad_u=grad_u + (1/m).*tmp.*(...
      -x(i).*tmpe_sigm)+...
       (1/m).*tmpe.*(tmpe_sigm+(2./5).*x(i).*tmpe_sigm+x(i).*a.*tmpe_sigm.*(1-tmpe_sigm));
   %Derivative of E with respect to the weights y_2
end
